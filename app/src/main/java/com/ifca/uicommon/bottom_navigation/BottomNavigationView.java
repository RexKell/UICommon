...
//            SHADOW_HEIGHT = (int) getResources().getDimension(R.dimen.bottom_navigation_shadow_height_without_colored_background);
        ...
        params.height = disableShadow ? NAVIGATION_HEIGHT : NAVIGATION_HEIGHT;
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        for (int i = 0; i < bottomNavigationItems.size(); i++) {
            ...
            TextView count=(TextView)view.findViewById(R.id.bottom_navigation_item_count);
            ...
            if (bottomNavigationItems.get(i).getNewsCount().equals("0")) {
                count.setVisibility(GONE);
            }else {
                count.setVisibility(VISIBLE);
                count.setText(bottomNavigationItems.get(i).getNewsCount());
            }
    ...
    public void setViewPager(ViewPager pager, int[] colorResources, int[] imageResources,String counts) {
        ...
        for (int i = 0; i < pager.getAdapter().getCount(); i++)
            addTab(new BottomNavigationItem(pager.getAdapter().getPageTitle(i).toString(), colorResources[i], imageResources[i],counts));
    ...
    /**设置代办的数目提示*/
    public void setTodoCount(String count){
        if (viewList.size()>0) {
            TextView todoCount = (TextView) viewList.get(0).findViewById(R.id.bottom_navigation_item_count);
            if (count.equals("0")){
                todoCount.setVisibility(GONE);
            }else {
                todoCount.setVisibility(VISIBLE);
            }
            todoCount.setText(count);
        }
    }
    /**设置消息的数目提示*/
    public void setNewsCount(String count){
        if (viewList.size()>0) {
            TextView newsCount = (TextView) viewList.get(1).findViewById(R.id.bottom_navigation_item_count);
            if(count.equals("0")){
                newsCount.setVisibility(GONE);
            }else {
                newsCount.setVisibility(VISIBLE);
            }
